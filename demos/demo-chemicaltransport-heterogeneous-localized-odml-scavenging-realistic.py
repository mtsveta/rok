...
sys.path.insert(1, '/home/skyas/polybox/allanleal-cpp-reactivetransportsolver-demo-restored/build-new/lib/python3.7/site-packages')
...
P = 200 * 1.01325 * 1e5  # the pressure (in units of Pa)

# Discretization parameters for the reactive transport simulation
lx = 100
ly = 30
nx = 200  # the number of mesh cells along the x-coordinate
...
tag_smart = "-" + activity_model + \
      ...
      "-nnodes-" + str(nnodes) + \
      "-nsteps-" + str(nsteps) + \
      "-reltol-" + "{:.{}e}".format(smart_equilibrium_reltol, 1) + \
      "-smart"
tag_conv = "-" + activity_model + \
      ...
      "-nsteps-" + str(nsteps) + \
      "-conv"
folder_results = 'results-new-sdhm-07.11.20/demo-scavenging-realistic'
...
def print_test_summary():
    ...
    print("ODML retol      :", smart_equilibrium_reltol)
...
smart_equilibrium_options.reltol = smart_equilibrium_reltol
...
def run_transport(use_smart_equilibrium):
    ...
    field.update()
    print("Initialization time: ", time.time() - t0_init)
